...
{
    BATTERY_LEVEL,
    NIMH_LIPO_BATTERY_VOLTAGE
} AM_batteryLevelDisplayType_t;
...
typedef enum
{
    SUNRISE_RECORDING,
    SUNSET_RECORDING,
    SUNRISE_AND_SUNSET_RECORDING,
    SUNSET_TO_SUNRISE_RECORDING,
    SUNRISE_TO_SUNSET_RECORDING
} AM_sunRecordingMode_t;
...
{
    INITIAL_GPS_FIX,
    GPS_FIX_BEFORE_RECORDING_PERIOD,
    GPS_FIX_BETWEEN_RECORDING_PERIODS,
    GPS_FIX_AFTER_RECORDING_PERIOD,
    GPS_FIX_BEFORE_INDIVIDUAL_RECORDING,
    GPS_FIX_BETWEEN_INDIVIDUAL_RECORDINGS,
...
static const configSettings_t defaultConfigSettings = {
    ...
    .nighttimeSampleRateChangeTime = 1769943600, // Sunday, February 1, 2026 11:00:00 AM (GMT) // actual 1784545200, // Monday, July 20, 2026 11:00:00 AM (GMT)
    .lowerFilterFreq = 0,
    .higherFilterFreq = 0,
...
static uint32_t formatDecibels(char *dest, uint32_t value, bool space)
{
    ...
    if (value > 0)
        return sprintf(dest, space ? "-%lu dB" : "-%ludB", value);
...
static void setHeaderComment(wavHeader_t *wavHeader, configSettings_t *configSettings, uint32_t currentTime, uint8_t *serialNumber, uint8_t *deploymentID, uint8_t *defaultDeploymentID, AM_extendedBatteryState_t extendedBatteryState, int32_t temperature, bool externalMicrophone, AM_recordingState_t recordingState, AM_filterType_t filterType)
{
    ...
    if (timezoneMinutes < 0)
        comment += sprintf(comment, ":%02d", -timezoneMinutes);
...
static uint32_t writeGuanoData(char *buffer, configSettings_t *configSettings, uint32_t currentTime, bool gpsLocationReceived, int32_t *gpsLastFixLatitude, int32_t *gpsLastFixLongitude, bool acousticLocationReceived, int32_t *acousticLatitude, int32_t *acousticLongitude, uint8_t *firmwareDescription, uint8_t *firmwareVersion, uint8_t *serialNumber, uint8_t *deploymentID, uint8_t *defaultDeploymentID, char *filename, AM_extendedBatteryState_t extendedBatteryState, int32_t temperature, AM_filterType_t filterType)
{
    ...
    if (gpsLocationReceived || acousticLocationReceived || configSettings->enableSunRecording)
    {
        ...
        int32_t latitude = gpsLocationReceived ? *gpsLastFixLatitude : acousticLocationReceived ? *acousticLatitude
                                                                                                : configSettings->latitude;

        int32_t longitude = gpsLocationReceived ? *gpsLastFixLongitude : acousticLocationReceived ? *acousticLongitude
                                                                                                  : configSettings->longitude;
    ...
    if (configSettings->enableLowGainRange)
        length += sprintf(buffer + length, " LGR");

    if (configSettings->disable48HzDCBlockingFilter)
        length += sprintf(buffer + length, " D48");

    if (isEnergySaverMode(configSettings))
        length += sprintf(buffer + length, " ESM");

    /* Battery and temperature */

    uint32_t batteryVoltage = extendedBatteryState == AM_EXT_BAT_LOW ? 24 : extendedBatteryState >= AM_EXT_BAT_FULL ? 50
                                                                                                                    : extendedBatteryState + AM_EXT_BAT_STATE_OFFSET / AM_BATTERY_STATE_INCREMENT;
...
static bool writeConfigurationToFile(configSettings_t *configSettings, uint32_t currentTime, bool gpsLocationReceived, int32_t *gpsLatitude, int32_t *gpsLongitude, bool acousticLocationReceived, int32_t *acousticLatitude, int32_t *acousticLongitude, uint8_t *firmwareDescription, uint8_t *firmwareVersion, uint8_t *serialNumber, uint8_t *deploymentID, uint8_t *defaultDeploymentID)
{
    ...
    else
    {
        ...
        if (configSettings->timezoneMinutes < 0)
            timezoneLength += sprintf(timezoneBuffer + timezoneLength, "-%d", configSettings->timezoneHours);
    ...
    if (configSettings->timezoneMinutes < 0)
        timezoneLength += sprintf(timezoneBuffer + timezoneLength, ":%02d", -configSettings->timezoneMinutes);
    ...
    if (configSettings->enableSunRecording)
    {
        ...
        int32_t latitude = gpsLocationReceived ? *gpsLatitude : acousticLocationReceived ? *acousticLatitude
                                                                                         : configSettings->latitude;

        int32_t longitude = gpsLocationReceived ? *gpsLongitude : acousticLocationReceived ? *acousticLongitude
                                                                                           : configSettings->longitude;
    ...
    length += sprintf(configBuffer + length, "Enable battery level indication : %s\r\n\r\n", configSettings->disableBatteryLevelDisplay ? "No" : configSettings->batteryLevelDisplayType == NIMH_LIPO_BATTERY_VOLTAGE ? "Yes (NiMH/LiPo voltage range)"
                                                                                                                                                                                                                      : "Yes");
    ...
    length += sprintf(configBuffer + length, "GPS fix before and after        : %s\r\n", configSettings->enableTimeSettingFromGPS == false ? "-" : configSettings->enableTimeSettingBeforeAndAfterRecordings ? "Individual recordings"
                                                                                                                                                                                                             : "Recording periods");
...
static void copyToBackupDomain(uint32_t *dst, uint8_t *src, uint32_t length)
{
    ...
    if (length % UINT32_SIZE_IN_BYTES)
...
static GPS_fixResult_t setTimeFromGPS(bool enableLED, AM_gpsFixMode_t fixMode, uint32_t timeout)
{
    ...
    if (result == GPS_CANCELLED_BY_MAGNETIC_SWITCH)
        writeGPSLogMessage(currentTime, currentMilliseconds, "Time was not updated. Cancelled by magnetic switch.");

    if (result == GPS_CANCELLED_BY_SWITCH)
        writeGPSLogMessage(currentTime, currentMilliseconds, "Time was not updated. Cancelled by switch position change.");

    if (result == GPS_TIMEOUT)
        writeGPSLogMessage(currentTime, currentMilliseconds, "Time was not updated. Timed out.");
...
int main(void)
{
    ...
    if (switchPosition != *previousSwitchPosition)
    {
        ...
        if (getBackupFlag(BACKUP_READY_TO_MAKE_RECORDING))
        {
            ...
            if (switchPosition == AM_SWITCH_CUSTOM && configSettings->enableTimeSettingFromGPS)
            {
                ...
                if (configSettings->requireAcousticConfiguration == false)
        ...
        if (getBackupFlag(BACKUP_READY_TO_MAKE_RECORDING))
        {
            ...
            if (isEnergySaverMode(configSettings))
                AudioMoth_setClockDivider(AM_HF_CLK_DIV2);
    ...
    if ((timeUntilNextGPSTimeSetting <= 0 || getBackupFlag(BACKUP_SHOULD_SET_TIME_FROM_GPS)) && timeUntilPreparationStart > GPS_MIN_TIME_SETTING_PERIOD * MILLISECONDS_IN_SECOND)
    {
        ...
        if (getBackupFlag(BACKUP_SHOULD_SET_TIME_FROM_GPS))
            fixMode += 1;

        if (timeUntilNextGPSTimeSetting > 0)
            fixMode += 1;
...
inline void GPS_handleTickEvent()
{
    ...
    if (gpsEnableLED)
        AudioMoth_setRedLED(gpsTickEventCount % gpsTickEventModulo == 0);
...
static AM_recordingState_t makeRecording(uint32_t timeOfNextRecording, uint32_t recordDuration, bool enableLED, AM_extendedBatteryState_t extendedBatteryState, int32_t temperature, uint32_t *fileOpenTime, uint32_t *fileOpenMilliseconds)
{
    ...
    if (AudioMoth_hasInvertedOutput())
    ...
    if (configSettings->enableDailyFolders)
    {
        ...
        if (directoryExists == false)
            FLASH_LED_AND_RETURN_ON_ERROR(AudioMoth_makeDirectory(foldername));
    }
    ...
    while (samplesWritten < numberOfSamples + numberOfSamplesInHeader && !microphoneChanged && !switchPositionChanged && !magneticSwitch && !supplyVoltageLow)
    {
        ...
        while (readBuffer != writeBuffer && samplesWritten < numberOfSamples + numberOfSamplesInHeader && !microphoneChanged && !switchPositionChanged && !magneticSwitch && !supplyVoltageLow)
        {
            ...
            if (frequencyTriggerEnabled && configSettings->sampleRateDivider > 1)
                writeIndicated = DigitalFilter_applyFrequencyTrigger(buffers[readBuffer], NUMBER_OF_SAMPLES_IN_BUFFER);
            ...
            else
            {
                ...
                if (shouldWriteThisSector)
                {
                    ...
                    if (buffersProcessed == 0)
                        memcpy(buffers[readBuffer], &wavHeader, sizeof(wavHeader_t));
    ...
    recordingState = microphoneChanged ? MICROPHONE_CHANGED : switchPositionChanged ? SWITCH_CHANGED
...
static void determineSunriseAndSunsetTimesAndScheduleRecording(uint32_t currentTime)
{
    ...
    if (configSettings->enableSunRecording && *timeOfNextSunriseSunsetCalculation == 0)
    {
        ...
        if (*timeOfNextSunriseSunsetCalculation < *timeOfNextRecording)
            *timeOfNextSunriseSunsetCalculation += SECONDS_IN_DAY;
    }
    else
    {
    ...
    if (configSettings->enableTimeSettingFromGPS == false)
        return;
...
static void determineTimeOfNextSunriseSunsetCalculation(uint32_t currentTime, uint32_t *timeOfNextSunriseSunsetCalculation)
{
    ...
    if (endOfGapMinutes < startOfGapMinutes)
        calculationMinutes += MINUTES_IN_DAY;
    ...
    if (calculationTime <= currentTime)
        calculationTime += SECONDS_IN_DAY;
...
static void determineSunriseAndSunsetTimes(uint32_t currentTime)
{
    ...
    float latitude = getBackupFlag(BACKUP_GPS_LOCATION_RECEIVED) ? (float)*gpsLatitude / (float)GPS_LOCATION_PRECISION : getBackupFlag(BACKUP_ACOUSTIC_LOCATION_RECEIVED) ? (float)*acousticLatitude / (float)ACOUSTIC_LOCATION_PRECISION
                                                                                                                                                                          : (float)configSettings->latitude / (float)CONFIG_LOCATION_PRECISION;

    float longitude = getBackupFlag(BACKUP_GPS_LOCATION_RECEIVED) ? (float)*gpsLongitude / (float)GPS_LOCATION_PRECISION : getBackupFlag(BACKUP_ACOUSTIC_LOCATION_RECEIVED) ? (float)*acousticLongitude / (float)ACOUSTIC_LOCATION_PRECISION
                                                                                                                                                                            : (float)configSettings->longitude / (float)CONFIG_LOCATION_PRECISION;
...
static void scheduleRecording(uint32_t currentTime, uint32_t *timeOfNextRecording, uint32_t *indexOfNextRecording, uint32_t *durationOfNextRecording, uint32_t *startOfRecordingPeriod, uint32_t *endOfRecordingPeriod)
{
    ...
    if (activeRecordingPeriods == 0)
    {
        ...
        *indexOfNextRecording = 0;
        ...
        if (endOfRecordingPeriod)
            *endOfRecordingPeriod = UINT32_MAX;
    ...
    if (startOfRecordingPeriod)
        *startOfRecordingPeriod = startTime;

    if (endOfRecordingPeriod)
        *endOfRecordingPeriod = startTime + duration;
    ...
    else
    {
        ...
        if (excessTime > 0)
        {
            ...
            *durationOfNextRecording -= excessTime;
            ...
            if (endOfRecordingPeriod)
                *endOfRecordingPeriod = *timeOfNextRecording + *durationOfNextRecording;
        }
    }
...
static void flashLedToIndicateBatteryLife(void)
{
    ...
    else
    {
        ...
        if (batteryState > AM_BATTERY_LOW)
        {
            ...
            numberOfFlashes = (batteryState >= AM_BATTERY_4V6) ? 4 : (batteryState >= AM_BATTERY_4V4) ? 3
                                                                 : (batteryState >= AM_BATTERY_4V0)   ? 2
                                                                                                      : 1;
        }
    }
